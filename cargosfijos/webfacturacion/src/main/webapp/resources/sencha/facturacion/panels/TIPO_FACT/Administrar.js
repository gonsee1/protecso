Ext.onReady(function(){
    fnc.load.ModelsAndStores(["TIPO_FACT"],function(){
        var package="Desktop.Facturacion.Panels.TIPO_FACT.Administrar";
        var AJAX_URI=settings.AJAX_URI+'TIPO_FACT/';
        var objDesktop=null;
        var storeData=new Desktop.Facturacion.Stores.TIPO_FACT();
        var varSendPackage=settings.Permissions.varSendPackage;
        fnc.addParameterStore(storeData,varSendPackage,package);    
        Ext.define(package,{ 
            extend:'Ext.grid.Panel',
            alias:'widget.Desktop.Facturacion.Panels.TIPO_FACT.Administrar',
            store:storeData,
            columns: [
                { text: 'Codigo',  dataIndex: 'CO_TIPO_FACT',width:'30px' },
                { text: 'Nombre', dataIndex: 'NO_TIPO_FACT', flex: 1,width:'30%' },
            ],
            width:'100%',
            tbar: [{
                xtype: 'button',
                    ...
                    click:function(){
                        ...
                        if(selected.length==1){
                            ...
                            fnc.createMsgConfirm({
                                msg:'Esta seguro que desea eliminar el item de modulo '+model.data.NO_TIPO_FACT,
                                call:function(btn){
                                    if(btn=='yes'){
                                        store.destroy({
                                            params:model.data,
            ...
            if(objDesktop!==null){ 
                ...
                store.load({
                    ...
                    callback:function(records,operation,success){
                        if (success){
                            var panelForm=Ext.create('Desktop.Facturacion.Panels.TIPO_FACT.Insertar',{
                                buttons: [  {
                                        text: 'Limpiar',
                                        handler: function() {
                                        ...
                                        handler: function() {
                                            ...
                                            if (form.isValid()) {
                                                ...
                                                params['CO_TIPO_FACT']=model.data.CO_TIPO_FACT;
                                                window.disable();
                                                model.save({
