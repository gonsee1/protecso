Ext.onReady(function(){
    fnc.load.scripts(["widgets/filterPagination.js"],function(){
       fnc.load.ModelsAndStores(["DEPA","PROV"],function(){
            var package="Desktop.Facturacion.Panels.UBIGEO.AdministrarProvincia";
            var AJAX_URI=settings.AJAX_URI;
            var objDesktop=null;
            var storeData=new Desktop.Facturacion.Stores.PROV();
            ...
            Ext.define(package,{ 
                ...
                alias:'widget.Desktop.Facturacion.Panels.UBIGEO.AdministrarProvincia',
                layout:'fit',
                tbar: [{
                    xtype: 'button',
                    text: "Editar",
                        ...
                        click:function(){
                            ...
                            if(selected.length==1){
                                ...
                                fnc.createMsgConfirm({
                                    msg:'Esta seguro que desea eliminar el item de modulo '+model.data.NO_PROV,
                                    call:function(btn){
                                        if(btn=='yes'){
                                            store.destroy({
                                                params:model.data,
                ...
                items:[    
                    { 
                        ...
                        store:storeData,
                        columns: [
                            { text: 'Codigo',  dataIndex: 'CO_PROV',width:'30px' },
                            { text: 'Nombre', dataIndex: 'NO_PROV', flex: 1,width:'30%' },
                            { text: 'Departamento', dataIndex: 'NO_DEPA', flex: 1,width:'30%',renderer:function(value, p, r){return r.data.DEPA.NO_DEPA;} },              
                        ],    
                    }    
                ],
                ...
                if(objDesktop!==null){ 
                    var store= new Ext.data.Store({
                        ...
                        proxy:{
                            ...
                            api: {
                                read:AJAX_URI+'PROV/getid/',
                                update:AJAX_URI+'PROV/update/',
                            },
                    ...
                    store.load({
                        ...
                        callback:function(records,operation,success){
                            if (success){
                                var panelForm=Ext.create('Desktop.Facturacion.Panels.UBIGEO.InsertarProvincia',{
                                    buttons: [  {
                                            text: 'Limpiar',
                                            handler: function() {
                                            ...
                                            handler: function() {
                                                ...
                                                if (form.isValid()) {
                                                    ...
                                                    params['CO_PROV']=model.data.CO_PROV;
                                                    window.disable();
                                                    model.save({
