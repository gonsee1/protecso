(function(){
    var package='Desktop.Facturacion.Widgets.filterPagination';  
    Ext.define(package,{
        ...
        initComponent:function(){
            ...
            thisObj.callParent();
            if (thisObj.store){
                fnc.load.scripts(["widgets/textFilterSearch.js"],function(){
                    var filterText=Ext.create({ xtype:'Desktop.Facturacion.Widgets.textFilterSearch',store:thisObj.store,onBeforeSearch:function(){
                        thisObj.store.currentPage=1;
                    }});
                    var pagination=Ext.create({ xtype: 'pagingtoolbar',displayInfo: true,width: '120',store:thisObj.store,listeners:{
                        beforechange:function(a,b,c){
                            fnc.addParameterStore(thisObj.store,"SEARCH",filterText.getValue());
                        }
                    }});
