Ext.onReady(function(){
    fnc.load.ModelsAndStores(["CONT_CLIE","TIPO_CLIE","TIPO_DOCU","SUCU","CLIE"],function(){
        fnc.load.scripts(["widgets/comboBoxDistrito.js"],function(){
        ...
        var storeClie_Valida=Ext.create('Desktop.Facturacion.Stores.CLIE',{autoLoad:false});
        fnc.addParameterStore(storeTipo_Docu,varSendPackage,package);
        fnc.addParameterStore(storeClie_Valida,varSendPackage,package);
        ...
        fnc.setApiProxyStore(storeClie_Valida,{read:'existeDocumento/'});
        storeRef.load({xtype:'combobox',
            callback:function(){
                Ext.define(package,{
                            ...
                            field_apellido,
                            {
                                ...
                                listeners:{
                                    select:function( combo, record, eOpts ){
                                        ...
                                        var data=record[0].data;
                                        var CO_TIPO_DOCU=fnc.parse.String.toInt(data.CO_TIPO_DOCU);
                                ...
                                listeners:{
                                    blur:function(){
                                        var f=this.up();
                                        var txtDocu=f.query("[name=DE_NUME_DOCU]")[0];
                                        var DE_NUME_DOCU=txtDocu.getValue();
                                        var CO_TIPO_DOCU=f.query("[name=CO_TIPO_DOCU]")[0].getValue();  
                                        if (CO_TIPO_DOCU){
                                            var params={
                                                    DE_NUME_DOCU:DE_NUME_DOCU,
                                                    CO_TIPO_DOCU:CO_TIPO_DOCU,
                                                };
                                            if(f.esEdicion)
                                                params["CO_CLIE"]=f.CLIE.CO_CLIE;
                                            else
                                                params["CO_CLIE"]=null;
                                            storeClie_Valida.load({
                                                params:params,
                                                callback:function (){
                                                    if (storeClie_Valida.data.items[0].data.existe){
                                                        fnc.createMsgBoxError({msg:"El número de documento ya existe."});
                                                        txtDocu.focus();
                                                    }
                                                }
                                            });
                                    ...
                                    items:[
                                        {
                                            ...
                                            displayField:'FULL_NAME_CONT_CLIE',//nombre y apellido
                        ...
                        handler: function() {
                            ...
                            if (form.isValid()) {
                                ...
                                for(var i=0;i<storeContactos.data.items.length;i++){
                                    ...
                                    if (r.DE_CORR == ''){
                                        data.CONT_DE_CORR.push('None');
                                    } else {
                                        data.CONT_DE_CORR.push(r.DE_CORR);
                                    }
                                    if (r.DE_TELE == ''){
                                        data.CONT_DE_TELE.push('None');
                                    } else {
                                ...
                                model.save({
                                    params:fnc.getParamsSendPackage(data,varSendPackage,package),
                                    callback:function(a,b,c,d,e){
                                        if(c){             
                                            var text = b._response.responseText;
